...
| `mlxserve-streamed.json` | **Live capture.** mlx-serve 0.1.0 wrapping `mlx_lm.server`. History head is a streamed request: real `ttft_ms` (96.2ms of 356.2ms) and a decode-only `tokens_per_second`. |
| `mlxserve-nonstreamed.json` | **Live capture**, same server, non-streamed. `ttft_ms` equals `total_duration_ms` (403 and 403) and the rate covers the whole request — the regression case for the stream detection. |
